...
class LlamaCppMetrics:
    ...
    def _compute_derived(self) -> None:
        ...
        if self.prompt_seconds_total > 0 and self.prompt_tokens_total > 0:
            self.avg_prompt_latency_ms = (
                self.prompt_seconds_total / self.prompt_tokens_total
            ) * 1000
        if self.tokens_predicted_seconds_total > 0 and self.tokens_predicted_total > 0:
            self.avg_generation_latency_ms = (
                self.tokens_predicted_seconds_total / self.tokens_predicted_total
            ) * 1000
...
class InferenceEngineMonitor:
    ...
    async def discover_instances(self) -> dict[str, LlamaCppInstance]:
        ...
        self._state.health_status = {
            iid: "healthy" if iid in active else "unhealthy" for iid in self.instances
        }
    ...
    async def collect_metrics(self, instance_id: str) -> LlamaCppMetrics | None:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                log.warning(
                    f"[InferenceEngine] Failed to fetch metrics from {instance_id}: {resp.status_code}"
                )
                return None
        except Exception as exc:
    ...
    async def collect_all_metrics(self) -> InferenceEngineState:
        ...
        if self._state.instances:
            ...
            self._state.avg_cache_hit_rate = sum(
                m.cache_hit_rate for m in self._state.instances.values()
    ...
    def get_state_summary(self) -> str:
        ...
        for instance_id, metrics in self._state.instances.items():
            ...
            lines.append(
                f"- **Generation throughput**: {metrics.predicted_tokens_seconds:.0f} tokens/sec"
            )
            lines.append(f"- **Max sequence length**: {metrics.n_tokens_max:.0f}")

            if instance.is_gpu:
                lines.append(
                    f"- **GPU VRAM**: {metrics.gpu_memory_used_mb:.0f}/{metrics.gpu_memory_total_mb:.0f} MB ({metrics.gpu_memory_used_mb / max(metrics.gpu_memory_total_mb, 1):.0%})"
                )

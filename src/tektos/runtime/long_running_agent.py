...
class CheckpointManager:
    ...
    async def cleanup_old_checkpoints(self, session_id: str, keep_last: int = 5) -> int:
        ...
        if removed:
            log.info(f"[Checkpoint] Cleaned up {removed} old checkpoints for session {session_id}")
    ...
    async def delete_session_checkpoints(self, session_id: str) -> int:
        ...
        log.info(f"[Checkpoint] Deleted {len(checkpoints)} checkpoints for session {session_id}")
...
class LongRunningAgent:
    ...
    async def resume(self) -> bool:
        ...
        if checkpoint:
            ...
            self.progress.status = "running"
            log.info(
                f"[LongRunningAgent] Resumed session {self.session_id} "
...
def get_long_running_agent(
    session_id: str, checkpoint_dir: str = "./checkpoints"
) -> LongRunningAgent:

...
class TavilyConfig(BaseModel):
    ...
    api_key: str = Field(default="", description="Tavily API key (TEKTOS_TAVILY_API_KEY env var)")
    base_url: str = Field(
        default="https://api.tavily.com/search", description="Tavily API base URL"
    )
    max_results: int = Field(default=10, description="Maximum results to return")
...
class SearXNGClient:
    ...
    async def search(self, query: str, max_results: int | None = None) -> SearchResponse:
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                if response.status_code != 200:
                    ...
                    logger.warning(
                        "SearXNG returned %d (attempt %d/%d)",
                ...
                logger.warning(
                    "SearXNG timed out (attempt %d/%d)", attempt + 1, self.config.max_retries
                )
                ...
                logger.warning(
                    "SearXNG parse error (attempt %d/%d): %s",
                    attempt + 1,
                    self.config.max_retries,
                    e,
                )
        ...
        return SearchResponse(
            query=query,
            error=f"SearXNG failed after {self.config.max_retries} attempts: {last_error}",
        )
    ...
    async def search(self, query: str, max_results: int | None = None) -> SearchResponse:
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                last_error = f"Request timed out: {e}"
                logger.warning(
                    "Tavily timed out (attempt %d/%d)", attempt + 1, self.config.max_retries
                )
            except httpx.RequestError as e:
                last_error = f"Network error: {e}"
                logger.warning(
                    "Tavily network error (attempt %d/%d): %s",
                    attempt + 1,
                    self.config.max_retries,
                    e,
                )
                ...
                logger.warning(
                    "Tavily parse error (attempt %d/%d): %s",
                    attempt + 1,
                    self.config.max_retries,
                    e,
                )
        ...
        return SearchResponse(
            query=query,
            error=f"Tavily failed after {self.config.max_retries} attempts: {last_error}",
        )
...
class UnifiedSearchProvider:
    ...
    def __init__(
        self, searxng_config: SearXNGConfig | None = None, tavily_config: TavilyConfig | None = None
    ):
    ...
    async def search(self, query: str, max_results: int | None = None) -> SearchResponse:
        ...
        if searxng_response.results:
            ...
            logger.info("SearXNG returned %d results for: %s", len(searxng_response.results), query)
            return searxng_response

        # SearXNG failed or returned no results — try Tavily
        if self.tavily_client:
            logger.info(
                "SearXNG failed or returned no results, falling back to Tavily for: %s", query
            )
...
_provider: UnifiedSearchProvider | None = None
...
async def unified_search(query: str, max_results: int | None = None) -> SearchResponse:
    """Execute a search query with SearXNG primary and Tavily fallback.

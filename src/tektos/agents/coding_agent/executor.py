...
class Executor:
    ...
    def execute_spec(self, spec: BuildSpec) -> ExecutionRecord:
        ...
        record.status = (
            ExecutionStatus.COMPLETED if not record.error_summary else ExecutionStatus.FAILED
        )
    ...
    def _generate_python_module(
        ...
        class_name = self._sanitize_filename(deliverable).replace("_", " ").title().replace(" ", "")
        lines = [
            f'"""{spec.description}',

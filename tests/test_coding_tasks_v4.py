"""
Tektos-Ultima v1 — Coding Tests (v4)

Improvements from v3:
1. Increased timeouts: easy=180s, medium=240s, hard=600s
2. Simplified hard test prompts
3. Progress checkpoints every 30s with file existence check
4. Hard tests run separately with extended timeout
...
# Timeouts by difficulty (increased from v3)
TIMEOUTS = {
    "easy": 180,
    "medium": 240,
    "hard": 600,  # Extended from 300s
}

# Progress monitoring interval
PROGRESS_INTERVAL = 30  # Check every 30 seconds
...
# ─── Hard Tests (3) — Simplified Prompts ───────────────────────────────────────
...
def test_avl_tree():
    ...
    prompt = """Write an AVL tree in Python at /tmp/avl_tree.py.

Requirements:
1. AVLNode: left, right, height, key, value
2. AVLTree: insert, delete, search, inorder_traversal
3. left_rotate, right_rotate
4. balance_factor()
5. main() inserts 10,20,30,40,50,25
...
def test_json_parser():
    """Hard: Recursive descent JSON parser (simplified)."""
    print("\n🔴 Hard 2: JSON Parser (Simplified)")
    session_id = create_session()
...
def test_task_pipeline():
    """Hard: Task pipeline with parallel execution (simplified)."""
    print("\n🔴 Hard 3: Task Pipeline (Simplified)")
    session_id = create_session()
...
4. main() with 5 stages (some parallel)
...
def main():
    print("=" * 60)
    print("Tektos-Ultima v1 — Coding Tests (v4)")
    print("Improvements: easy=180s, medium=240s, hard=600s")
    print("Progress checkpoints every 30s, simplified hard prompts")
    print("=" * 60)
